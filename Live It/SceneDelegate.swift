...
class SceneDelegate: UIResponder, UIWindowSceneDelegate, UIGestureRecognizerDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let languageManager: LocalizationService = LanguageManager(languageCode: .english)
        DispatchQueue.main.async {
            let baseVC = OnboardController()
            //baseVC.selectedIndex = 0
            baseVC.localizeService = languageManager
            //debugPrint("current language scene : \(languageManager.currentLanguage.rawValue)")
            let navigationController = UINavigationController(rootViewController: baseVC)
            navigationController.isNavigationBarHidden = true
            self.setupNaigationController(navigationController)
        }
    }
    func setupNaigationController(_ controller: UINavigationController){
